"""
Indonesia Kulamagi Strategy - Complete Implementation
Implementasi lengkap strategi Christian Kulamagi untuk pasar Indonesia
dengan integrasi database MySQL dan Yahoo Finance
"""
import pymysql
import pandas as pd
import numpy as np
...
class IndonesiaKulamagiComplete:
    """
    Implementasi lengkap strategi Christian Kulamagi untuk pasar Indonesia
    """
    ...
    def __init__(self, host='localhost', user='root', password='', database='scalper'):
        ...
        # Strategy parameters for Indonesia market
        ...
        # Indonesia market specific parameters
        self.momentum_threshold_1m = 0.10  # 10% for 1 month
        self.momentum_threshold_3m = 0.20  # 20% for 3 months
        self.momentum_threshold_6m = 0.30  # 30% for 6 months
        self.breakout_move_min = 15        # 15% minimum move
        self.breakout_move_max = 60        # 60% maximum move
        self.consolidation_range_max = 25  # 25% maximum consolidation range
    ...
    def get_database_stocks(self):
        ...
        try:
            ...
            with self.connection.cursor() as cursor:
                # Get stocks from idx_stocks table
                cursor.execute("SELECT symbol, name, sector FROM idx_stocks WHERE is_active = 1 LIMIT 50")
                ...
                # Fallback to historical_ohlcv_daily
                cursor.execute("SELECT DISTINCT symbol FROM historical_ohlcv_daily LIMIT 50")
                symbols = cursor.fetchall()
                
                if symbols:
                    logger.info(f"✅ Found {len(symbols)} symbols in historical data")
                    return [{'symbol': s['symbol'], 'name': s['symbol'], 'sector': 'Unknown'} for s in symbols]
    ...
    def get_database_market_data(self, symbol: str, days: int = 365):
    ...
    def screen_indonesia_momentum_stocks(self, symbols: List[str] = None):
        ...
        if symbols is None:
            ...
            if db_stocks:
                symbols = [stock['symbol'] for stock in db_stocks[:30]]  # Top 30 from database
            else:
                # Fallback to popular Indonesia stocks
                symbols = [
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                if (performance_1m >= self.momentum_threshold_1m and 
                    performance_3m >= self.momentum_threshold_3m and 
                    performance_6m >= self.momentum_threshold_6m):
    ...
    def _find_momentum_leg(self, df: pd.DataFrame):
        ...
        for i in range(10, len(df)):
            ...
            if self.breakout_move_min <= move_percent <= self.breakout_move_max:
                return {
    ...
    def _find_consolidation_phase(self, df: pd.DataFrame, momentum_leg: Dict):
        ...
        if range_percent > self.consolidation_range_max:
            return None
    ...
    def calculate_position_size(self, portfolio_value: float, entry_price: float, stop_loss: float):
    ...
    def run_complete_analysis(self):
        ...
        logger.info("🚀 Starting Complete Indonesia Kulamagi Strategy Analysis")
        logger.info("=" * 70)
        ...
        if momentum_stocks:
            example_stock = momentum_stocks[0]
            portfolio_value = 1000000  # 1M IDR portfolio
            entry_price = example_stock['current_price']
            stop_loss = entry_price * 0.95  # 5% stop loss
            
            position_size = self.calculate_position_size(portfolio_value, entry_price, stop_loss)
            
            if "error" not in position_size:
                logger.info(f"Example for {example_stock['symbol']}:")
                logger.info(f"  Portfolio Value: {portfolio_value:,.0f} IDR")
                logger.info(f"  Entry Price: {entry_price:.2f}")
                logger.info(f"  Stop Loss: {stop_loss:.2f}")
                logger.info(f"  Shares: {position_size['shares']}")
                logger.info(f"  Position Value: {position_size['position_value']:,.0f} IDR")
                logger.info(f"  Risk Amount: {position_size['risk_amount']:,.0f} IDR")
                logger.info(f"  Risk %: {position_size['risk_percent']:.2f}%")
        
        # 6. Summary
        logger.info("\n📋 COMPLETE ANALYSIS SUMMARY")
        logger.info("=" * 70)
        logger.info(f"Database: {self.database} (Connected: {db_connected})")
        ...
        logger.info(f"  - Risk per Trade: {self.risk_per_trade_min:.2%} - {self.risk_per_trade_max:.2%}")
        logger.info(f"  - Max Position Size: {self.max_position_size:.1%}")
        logger.info(f"  - Momentum Thresholds: 1M≥{self.momentum_threshold_1m:.0%}, 3M≥{self.momentum_threshold_3m:.0%}, 6M≥{self.momentum_threshold_6m:.0%}")
        ...
        if market_condition['market_favorable'] and momentum_stocks:
            ...
            logger.info("✅ Take partial profits (1/3) after 3-5 days")
...
def main():
    """Main function to run complete Indonesia Kulamagi analysis"""
    analyzer = IndonesiaKulamagiComplete()
    result = analyzer.run_complete_analysis()
    
    if result:
        logger.info("\n✅ Complete analysis finished successfully!")
        logger.info(f"📊 Results: {result['breakout_count']} breakout setups, {len(result['momentum_stocks'])} momentum stocks")
        return result
    else:
        logger.error("\n❌ Analysis failed!")
        return None
