#!/usr/bin/env python3
"""
Test Historical Data - Fixed Version
======================================

Script untuk testing dengan data historis asli minimal 6 bulan
dengan perbaikan error handling dan JSON serialization.
...
def test_historical_data_fixed():
    """Test dengan data historis asli minimal 6 bulan - Fixed Version"""
    print("HISTORICAL DATA TESTING - FIXED VERSION")
    print("=" * 70)
    ...
    # Test results
    test_results = {
        'test_type': 'historical_data_testing_fixed',
    ...
    historical_analysis = analyze_historical_data_fixed(cursor)
    ...
    six_month_test = test_six_months_historical_data_fixed(cursor)
    ...
    performance_metrics = calculate_real_performance_metrics_fixed(cursor, historical_analysis)
    ...
    enhanced_modules_test = test_enhanced_modules_with_historical_data_fixed(cursor)
    ...
    real_accuracy = generate_real_accuracy_report_fixed(test_results)
    ...
    generate_historical_test_report_fixed(test_results)
    
    return test_results

def analyze_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Analyze historical data availability - Fixed Version"""
    try:
        analysis = {
        ...
        for table in historical_tables:
            try:
                ...
                if total_records > 0:
                    ...
                    # Get date range - handle different column names
...
def test_six_months_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test with 6 months of historical data - Fixed Version"""
    try:
        test_results = {
            'trading_performance': {},
        ...
        trading_perf = test_trading_with_historical_data_fixed(cursor)
        ...
        market_data_perf = test_market_data_with_historical_data_fixed(cursor)
        ...
        risk_perf = test_risk_management_with_historical_data_fixed(cursor)
...
def test_trading_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test trading performance with historical data - Fixed Version"""
    try:
        ...
        total_orders = int(trading_stats[0]) if trading_stats[0] else 0
        total_trades = int(trade_stats[0]) if trade_stats[0] else 0
        ...
        return {
            ...
            'avg_price': float(trading_stats[1]) if trading_stats[1] else 0.0,
            'total_quantity': int(trading_stats[2]) if trading_stats[2] else 0,
            'unique_symbols': int(trading_stats[3]) if trading_stats[3] else 0
        }
...
def test_market_data_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test market data performance with historical data - Fixed Version"""
    try:
        ...
        # Get market data quality - check if price column exists
        try:
            cursor.execute("""
                SELECT 
            ...
            # Fallback if price column doesn't exist
            cursor.execute("""
                SELECT 
                    COUNT(*) as total_market_data,
                    COUNT(DISTINCT symbol) as unique_symbols
            ...
            """)
            market_data_stats = cursor.fetchone()
            market_data_stats = (market_data_stats[0], 0.0, market_data_stats[1])
        
        # Calculate market data performance
        total_records = int(market_stats[0]) if market_stats[0] else 0
        total_market_data = int(market_data_stats[0]) if market_data_stats[0] else 0
        ...
        return {
            ...
            'unique_symbols': int(market_stats[3]) if market_stats[3] else 0
        }
...
def test_risk_management_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test risk management performance with historical data - Fixed Version"""
    try:
        # Get historical risk data - check if risk_metrics table exists and has data
        try:
            cursor.execute("""
                SELECT 
            ...
            # Fallback if risk_metrics table doesn't exist or has different structure
            cursor.execute("SELECT COUNT(*) FROM risk_metrics")
            risk_count = cursor.fetchone()[0]
            risk_stats = (risk_count, 0.0, 0)
        ...
        try:
            ...
            # Fallback if portfolio_risk table doesn't exist or has different structure
            cursor.execute("SELECT COUNT(*) FROM portfolio_risk")
            portfolio_risk_count = cursor.fetchone()[0]
            portfolio_risk_stats = (portfolio_risk_count, 0.0, 0)
        
        # Calculate risk management performance
        total_risk_metrics = int(risk_stats[0]) if risk_stats[0] else 0
        total_portfolio_risk = int(portfolio_risk_stats[0]) if portfolio_risk_stats[0] else 0
        ...
        return {
            ...
            'accuracy': float(accuracy),
            'avg_risk_score': float(risk_stats[1]) if risk_stats[1] else 0.0,
            'unique_portfolios': int(risk_stats[2]) if risk_stats[2] else 0
        }
...
def test_enhanced_modules_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test enhanced modules with historical data - Fixed Version"""
    try:
        ...
        enhanced_trading = test_enhanced_trading_with_historical_data_fixed(cursor)
        ...
        enhanced_market_data = test_enhanced_market_data_with_historical_data_fixed(cursor)
        ...
        enhanced_risk_management = test_enhanced_risk_management_with_historical_data_fixed(cursor)
        ...
        integration_tests = test_integration_with_historical_data_fixed(cursor)
...
def test_enhanced_trading_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test enhanced trading module with historical data - Fixed Version"""
    try:
        ...
        total_orders = int(order_stats[0]) if order_stats[0] else 0
        total_trades = int(trade_stats[0]) if trade_stats[0] else 0
        ...
        return {
            ...
            'execution_rate': float(execution_rate),
            'accuracy': float(accuracy),
            'avg_price': float(order_stats[1]) if order_stats[1] else 0.0,
            'total_quantity': int(order_stats[2]) if order_stats[2] else 0,
            'unique_symbols': int(order_stats[3]) if order_stats[3] else 0
        }
...
def test_enhanced_market_data_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test enhanced market data module with historical data - Fixed Version"""
    try:
...
def test_enhanced_risk_management_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test enhanced risk management module with historical data - Fixed Version"""
    try:
...
def test_integration_with_historical_data_fixed(cursor) -> Dict[str, Any]:
    """Test module integration with historical data - Fixed Version"""
    try:
        integration_results = {}
        ...
        try:
            ...
            integration_results['trading_market_data'] = {'success': True, 'count': int(count)}
        ...
        try:
            ...
            """)
            count = cursor.fetchone()[0]
            integration_results['risk_management'] = {'success': True, 'count': int(count)}
...
def calculate_real_performance_metrics_fixed(cursor, historical_analysis: Dict[str, Any]) -> Dict[str, Any]:
    """Calculate real performance metrics - Fixed Version"""
    try:
        metrics = {
            'data_availability': {},
            'data_quality': {},
        ...
        metrics['data_availability'] = {
            ...
            'total_records': total_records,
            'availability_score': float(min(total_tables / 10, 1.0) * 100)
        }
        ...
        metrics['performance_scores'] = {
            'data_availability_score': float(metrics['data_availability']['availability_score']),
            'data_quality_score': float(avg_quality),
            'overall_performance': float((metrics['data_availability']['availability_score'] + avg_quality) / 2)
        }
...
def generate_real_accuracy_report_fixed(test_results: Dict[str, Any]) -> Dict[str, Any]:
    """Generate real accuracy report - Fixed Version"""
    try:
        accuracy_report = {
        ...
        accuracy_report['historical_data_accuracy'] = {
            'trading': float(trading_perf.get('accuracy', 0)),
            'market_data': float(market_data_perf.get('accuracy', 0)),
            'risk_management': float(risk_perf.get('accuracy', 0))
        }
        ...
        accuracy_report['enhanced_modules_accuracy'] = {
            'enhanced_trading': float(enhanced_trading.get('accuracy', 0)),
            'enhanced_market_data': float(enhanced_market_data.get('accuracy', 0)),
            'enhanced_risk_management': float(enhanced_risk_management.get('accuracy', 0))
        }
        
        # Calculate overall accuracy
        all_accuracies = [
            float(trading_perf.get('accuracy', 0)),
            float(market_data_perf.get('accuracy', 0)),
            float(risk_perf.get('accuracy', 0)),
            float(enhanced_trading.get('accuracy', 0)),
            float(enhanced_market_data.get('accuracy', 0)),
            float(enhanced_risk_management.get('accuracy', 0))
        ]
        
        accuracy_report['overall_accuracy'] = float(sum(all_accuracies) / len(all_accuracies))
...
def generate_historical_test_report_fixed(test_results: Dict[str, Any]) -> None:
    """Generate historical test report - Fixed Version"""
    ...
    results_file = f"historical_data_test_fixed_{timestamp}.json"
