...
Script trading yang sudah diperbaiki dengan:
...
def calculate_indicators(prices):
    ...
    if len(prices) < 20:
        ...
        synthetic_prices = [base_price * random.uniform(0.95, 1.05) for _ in range(50)]
...
def generate_trading_signals(indicators, current_price):
    """Generate trading signals with FIXED logic"""
    signals = []
    signal_score = 0
    
    # Trend Analysis (40% weight) - FIXED
    trend_score = 0
    ...
    # Momentum Analysis (30% weight) - FIXED
    momentum_score = 0
    ...
    # Price Action Analysis (30% weight) - FIXED
    price_score = 0
    ...
    # Calculate total score - FIXED
    total_score = (trend_score * 0.4) + (momentum_score * 0.3) + (price_score * 0.3)
    
    # Determine action with FIXED thresholds
    if total_score >= 1.5:  # LOWERED from 2.5
        return 'STRONG_BUY', signals, total_score
    elif total_score >= 0.8:  # LOWERED from 1.0
        return 'BUY', signals, total_score
    elif total_score <= -1.5:  # LOWERED from -2.5
...
def calculate_position_size(account_balance, signal_strength, current_price, portfolio_diversification=0.8):
    """Calculate position size with FIXED logic"""
    ...
    # FIXED: Adjust for signal strength with proper ranges
    if signal_strength >= 2.0:
        strength_multiplier = 2.0  # Strong signals get 2x position
    elif signal_strength >= 1.5:
        strength_multiplier = 1.5  # Medium-strong signals get 1.5x position
    elif signal_strength >= 1.0:  # FIXED: Added this range
        strength_multiplier = 1.0  # Medium signals get 1x position
    elif signal_strength >= 0.5:  # FIXED: Added this range
        strength_multiplier = 0.5  # Weak signals get 0.5x position
    else:
        strength_multiplier = 0.2  # Very weak signals get 0.2x position
...
def simulate_fixed_monthly_trading(cursor, symbols, account_balance, month, year, portfolio):
    """Simulate fixed trading for one month"""
    print(f"\n   === FIXED MONTH {month:02d}/{year} ===")
    ...
    for day in range(1, trading_days + 1):
        ...
        if historical_data:
            ...
            indicators = calculate_indicators(synthetic_prices)
        
        # Generate signals
        action, signals, signal_strength = generate_trading_signals(indicators, current_price)
        ...
        if symbol in portfolio:
            ...
            if pnl_percent <= -5 or pnl_percent >= 10:
                # Close position
                trade = execute_trade(symbol, 'SELL' if position['action'] == 'BUY' else 'BUY', quantity, current_price)
                if trade:
        ...
        if action in ['BUY', 'STRONG_BUY', 'SELL', 'STRONG_SELL']:
            ...
            quantity = calculate_position_size(account_balance, signal_strength, current_price)
...
def main():
    ...
    print(f"Signal Thresholds: BUY >= 0.8, STRONG_BUY >= 1.5")
    print("=" * 80)
    ...
    try:
        ...
        print(f"[INFO] Using {len(symbols)} symbols for fixed trading")
        ...
        # Simulate 12 months with fixes
        for month in range(1, 13):
            print(f"\nFIXING MONTH {month:02d}/{simulation_year}")
            print("-" * 60)
            
            month_result = simulate_fixed_monthly_trading(cursor, symbols, account_balance, month, simulation_year, portfolio)
        ...
        results = {
            'simulation_period': f"1 year ({simulation_year}) - FIXED",
        ...
        print("\n" + "=" * 80)
        print("FIXED TRADING SIMULATION RESULTS")
        print("=" * 80)
        print(f"Simulation Period: 1 year ({simulation_year}) - FIXED")
        print(f"Initial Balance: Rp {initial_balance:,}")
        ...
        print("\nBUG FIXES IMPLEMENTED:")
        print("-" * 80)
        print("1. FIXED POSITION SIZING:")
        print("   - Added proper signal strength ranges (0.5, 1.0, 1.5, 2.0+)")
        print("   - Fixed strength_multiplier calculation")
        print("   - Added minimum position size for weak signals")
        
        print("\n2. FIXED SIGNAL GENERATION:")
        print("   - Lowered BUY threshold from 1.0 to 0.8")
        print("   - Lowered STRONG_BUY threshold from 2.5 to 1.5")
        print("   - Added proper signal strength calculation")
        
        print("\n3. FIXED RISK MANAGEMENT:")
        print("   - Proper stop loss (5%) and take profit (10%)")
        print("   - Position size limits (5% max per position)")
        print("   - Portfolio diversification (80%)")
        
        print("\n4. FIXED PORTFOLIO MANAGEMENT:")
        ...
        output_filename = f"fixed_trading_simulation_{file_timestamp}.json"
        with open(output_filename, "w") as f:
            json.dump(results, f, indent=4, default=str)
        
        print(f"\nResults saved to: {output_filename}")
        print(f"Bug fixes completed at: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
