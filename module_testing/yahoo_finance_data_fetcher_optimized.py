#!/usr/bin/env python3
"""
Yahoo Finance Data Fetcher Optimized
===================================

Script untuk mengambil data saham dari Yahoo Finance dengan simbol yang valid,
terutama saham Indonesia, dan melakukan testing yang komprehensif.
...
class YahooFinanceDataFetcherOptimized:
    """Yahoo Finance Data Fetcher Optimized for Indonesian Stocks"""
    ...
    def __init__(self, db_config: Dict[str, Any]):
        ...
        self.batch_delay = 10.0   # 10 seconds between batches
        self.max_retries = 3
        self.timeout = 30
        
        # Valid Indonesian stock symbols (IDX) - Tested and verified
        self.valid_indonesian_stocks = [
            # Banking - Major banks
            ...
            # Finance
            'BTPN.JK',  # Bank BTPN
            'BKSW.JK'   # Bank Woori Saudara
        ]
        
        # Limit to 20 stocks for optimal testing
        self.valid_indonesian_stocks = self.valid_indonesian_stocks[:20]
    ...
    def populate_market_data(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            insert_query = """
                INSERT INTO market_data (symbol, timestamp, date, price, volume, high, low, open, close, created_at)
    ...
    def fetch_all_indonesian_stocks(self) -> Dict[str, Any]:
        ...
        print("FETCHING INDONESIAN STOCK DATA - OPTIMIZED VERSION")
        print("=" * 80)
        print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Total stocks: {len(self.valid_indonesian_stocks)}")
        print(f"Stocks: {', '.join(self.valid_indonesian_stocks)}")
        print("=" * 80)
        
        results = {
            'test_type': 'yahoo_finance_data_fetch_optimized',
        ...
        # Process each stock
        for i, symbol in enumerate(self.valid_indonesian_stocks, 1):
            print(f"\n{i}/{len(self.valid_indonesian_stocks)} Processing {symbol}")
            print("-" * 60)
        ...
        # Generate summary
        results['summary'] = {
            'total_stocks': len(self.valid_indonesian_stocks),
    ...
    def generate_fetch_report(self, results: Dict[str, Any]) -> None:
        ...
        print("\nYAHOO FINANCE DATA FETCH REPORT - OPTIMIZED VERSION")
        print("=" * 80)
        ...
        results_file = f"yahoo_finance_fetch_optimized_{timestamp}.json"
...
def main():
    ...
    fetcher = YahooFinanceDataFetcherOptimized(db_config)
